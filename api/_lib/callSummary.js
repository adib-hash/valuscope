...
- exchanges: one entry per numbered exchange, in order, with its number. question is the substance of what the analyst asked in one or two sentences; when they asked two things, keep both. answer is the substance of the reply in two to four sentences, keeping every number, range, percentage, date and timing commitment exactly as given and dropping the pleasantries. responder is the executive who actually answered; an executive who added to the answer goes in alsoAnswered. directness is direct when the question was answered as asked; partial when only part was addressed, or the answer was general where the question was specific; deflected when the reply moved to a different subject; declined when they said they do not disclose, do not break that out, or would not comment. newInformation is true only when the answer contained something not already in prepared remarks. pushback is true when the executive disagreed with the analyst's premise.
...
- quotes: six to ten short verbatim passages spoken by executives, each under 25 words, copied exactly: punctuation, contractions, hesitations and all. Prefer lines that carry tone, conviction or hedging over lines that carry numbers. Never paraphrase, never trim mid-sentence, never clean up wording. A quote that is not exactly in the transcript will be discarded.
- topics: the six to ten subjects the call spent the most time on, as short labels, so the next quarter's call can be compared with this one.
- newMentions: the products, segments, customers, markets, partners and programmes named on the call, as short labels (at most fifteen), so the next quarter can tell which are new.
- Be complete on facts and terse in wording: this output is read by a model, not a person.`;
