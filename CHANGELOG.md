...
### Notes
- A summary is now two or three Gemini calls (extraction, the prior quarter's extraction when none is stored, composition) instead of one, so it takes about half a minute rather than ten seconds; the prior-quarter step runs in parallel on a 30-second clock and the summary goes out without "what changed" rather than not at all if it overruns. Ordinary transcripts fit one extraction call; one that exceeded the budget would be chunked by exchange, never by size.
