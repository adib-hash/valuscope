...
- **The calendar's transcript index could fail with a DNS `EBUSY`.** Reading the index columns in one call fanned out a range request per column chunk per row group at once; the runtime's resolver refused. Row groups are now read eight at a time, sequentially, with one retry.
