...
- **Call summaries are rebuilt around the Q&A.** The single prompt over a raw transcript was good on the numbers and weak on the Q&A, which is where the signal lives. The summary is now a two-pass pipeline: the transcript is split into prepared remarks and Q&A on the operator's cues, and the Q&A into exchanges, so a question is never separated from its answer; a structured extraction pass records every speaker with title, every exchange with who asked, who answered, the substance and how direct the answer was, every guidance figure with its prior value, verbatim quotes and the topics covered; the prose is then written from that extraction, never from the raw transcript. "What Analysts Pressed On" becomes a paired **Q&A** section (analyst and firm, executive and title, the answer with its numbers, and a one-line read on directness), joined by **What Management Didn't Answer** (deflected, generic or declined questions), **Notable Quotes** (verbatim, attributed, and checked against the transcript — a quote that is not in it, word for word, is dropped, in both passes), and **What Changed** against the prior quarter's call (guidance revisions old to new, new products or customers, topics that dropped out, tone) — present only when a prior call exists, never guessed. Every quantitative claim carries its number, filler adjectives are banned outside quotes, statements are attributed by name and title, and the whole thing is held to roughly 900 words. Key Takeaways, Financial Highlights, Guidance and Risks are unchanged. Headline figures also arrive as structured label/value/comparison chips.
- **Old versus new, side by side.** `node scripts/compare-summary.mjs DELL 2027 1` runs the retired prompt and the new pipeline against the same transcript and prints both as Markdown with word counts, timings, the exchanges found and the quotes verified.
- **Call summaries persist.** Each summary is written once to the artifact store (`summary/v2/{symbol}/{year}-Q{quarter}.json`, with the extraction beside it under `extract/v2/`) and served from there ever after, so a digest re-opened tomorrow, or by someone else, pays only for calls not yet read — and the next quarter's "what changed" reads the stored extraction instead of the old transcript. The day-level read is stored the same way, keyed by the set of calls it covered, so a day whose transcripts arrive over two mornings gets a fresh read once the set grows.

### Fixed
- **Transcripts and the transcript index broke when the dataset moved.** On 15 September the defeatbeta dataset moved its US tables from `data/` to `data/US/`, so the transcript file 404ed: every scheduled `Transcript index` run failed at the first request and `/api/transcript` lost its primary source. Both now read the new path, from one exported constant, and the index build names the missing URL and where to look instead of hyparquet's bare `fetch head failed 404`.
- **Summaries timed out (504) on Vercel.** The two-pass pipeline ran its two or three Gemini calls inside one request, and on a long call (Dell Q1 FY27) the extractions used up most of the function's sixty seconds before composition had started. Each pass is now its own request — `stage=extract`, `stage=prior`, `stage=compose` on `/api/summarize` — and the client carries the results between them, so no request holds more than one model call. That also means a deployment without a Blob store works: nothing has to persist server-side between stages. The current and prior extractions run in parallel from the browser, the single-call view shows which stage it is on, and finished summaries are kept in localStorage (forty most recent) so a re-open is instant on the same device even where nothing persists server-side.
- **The calendar could not tell which calls had transcripts.** "The transcript dataset is unreachable" on every load: the index columns are spread across hundreds of row groups, so one scan is thousands of range requests, and the dataset's CDN answered the burst with a DNS `EBUSY` and then 429 — inside a function that has sixty seconds regardless. The index is no longer read at request time at all. A scheduled workflow (`transcript-index.yml`, every six hours, or on demand) reads the dataset slowly — two requests in flight, backoff on 429 — and commits `data/transcript-index.json` (S&P 500 constituents, last two years, about 150 KB) when the set of calls has changed; the function reads that file, from the repository first and the bundled copy otherwise. The grid footer now says when transcripts were last indexed, and an unbuilt or empty index says so instead of blaming the dataset.

### Notes
- Ordinary transcripts fit one extraction call; one that exceeded the budget would be chunked by exchange, never by size. `GEMINI_THINKING_LEVEL` (e.g. `low`) trades the model's reasoning for speed on the pipeline's calls; unset, nothing is sent.
- Production has no Blob store, so every summary is three Gemini calls and "what changed" always re-reads the prior transcript. Creating one (Storage → Create → Blob) makes repeat summaries free and the prior-quarter comparison a store read.
- Still twelve functions. The calendar is `op=calendar` on `/api/earnings`, the digest is `op=digest` on `/api/summarize`; both functions now carry the 60-second duration cap. Five hundred companies is five batch quotes, not five hundred calls, and transcript availability comes from the committed index file, held in memory for an hour.
